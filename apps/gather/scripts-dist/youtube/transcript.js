...
async () => {
    ...
    if (!track)
        track = tracks.find((item) => item?.kind !== "asr") || tracks[0];
    if (!track?.baseUrl)
        throw new Error("Caption URL unavailable");
    ...
    if (!segments.length)
        throw new Error("Parsed 0 segments from caption XML");
    if (mode === "raw") {
        return {
            mode,
    ...
    for (const seg of segments) {
        ...
        if (sentenceEnd.test(seg.text))
            flush();
        if (rows.length >= __COUNT__)
            break;
    }
    if (rows.length < __COUNT__)
        flush();
