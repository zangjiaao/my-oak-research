...
async () => {
    ...
    if (!query)
        return { error: "Missing argument: query", hint: "Provide a search keyword" };
    ...
    if (!response.ok)
        return { error: `HTTP ${response.status}`, hint: "Open so.toutiao.com first" };
    ...
    const extractArticleUrl = (href) => {
        ...
        try {
            ...
            for (let i = 0; i < 5; i += 1) {
                ...
                if (groupMatch)
                    return `https://www.toutiao.com/article/${groupMatch[1]}/`;
    ...
    for (const card of Array.from(doc.querySelectorAll(".cs-card"))) {
        ...
        if (!title || title.length < 2)
            continue;
        if (title.includes("去西瓜搜") || title.includes("去抖音搜"))
            continue;
        ...
        if (titleIdx2 >= 0)
            rest = rest.substring(titleIdx2 + title.length);
    ...
    if (results.length === 0) {
        return { error: "No results found", hint: "Toutiao may require login", query };
    }
