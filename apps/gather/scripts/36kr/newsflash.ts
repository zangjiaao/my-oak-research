...
async () => {
  ...
  if (!response.ok) {
    ...
    if (!pageResponse.ok) return { error: `HTTP ${pageResponse.status}`, hint: "Navigate to 36kr.com first" };
  ...
  if (payload?.code !== 0) return { error: `API error: ${payload?.msg || payload?.code || "unknown"}` };
